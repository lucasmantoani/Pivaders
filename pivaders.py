...
# Ici, on met en place quelques variables globales faire des modifications plus facilement sur des éléments récurrents 
BLACK = (0, 0, 0)
BLUE = (0, 0, 255)
WHITE = (255, 255, 255)
RED = (255, 0, 0)
...
class Game(object):
    ...
    def splash_screen(self):
        while GameState.start_screen:
            ...
            self.screen.blit(self.game_font.render(
                "PRESS SPACE TO START THE WAR !", 1, WHITE), (220, 200))
            self.screen.blit(self.game_font.render(
                "Lucas Mantoani - 2019", 1, BLACK), (280, 400))
            pygame.display.flip()
            self.control()
    ...
    def refresh_scores(self):
        ...
        self.screen.blit(self.game_font.render(
            "VIES " + str(self.lives + 1), 1, RED), (355, 575))
    ...
    def is_dead(self):
        if self.lives < 0:
            self.screen.blit(self.game_font.render(
                "Bataille perdue ! Votre score: " + str(
    ...
    def defenses_breached(self):
        for alien in self.alien_group:
            if alien.rect.y > 410:
                self.screen.blit(self.game_font.render(
                    "Les aliens ont brisé les défenses terrestres !",
                    1, RED), (180, 15))
                self.refresh_screen()
                self.level_up = 50
                self.explode = False
    ...
    def win_round(self):
        if len(self.alien_group) < 1:
            self.rounds_won += 1
            self.screen.blit(self.game_font.render(
                "fin du round " + str(self.rounds_won) +
                "  mais la bataille fait rage...", 1, RED), (200, 150))
            self.refresh_screen()
            pygame.time.delay(5000)
            return True
